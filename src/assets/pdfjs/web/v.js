...
function renderPage(activeServiceOnEntry, pdfDocument, pageNumber, size) {
  ...
  // });
  var scratchCanvas = activeService.scratchCanvas;
  // Using a lower PRINT_RESOLUTION for mobile might improve performance
  var PRINT_RESOLUTION = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ? 96 : 150;
  var PRINT_UNITS = PRINT_RESOLUTION / 72.0;
  scratchCanvas.width = Math.floor(size.width * PRINT_UNITS);
  scratchCanvas.height = Math.floor(size.height * PRINT_UNITS);
  var width = Math.floor(size.width * _ui_utils.CSS_UNITS) + 'px';
...
PDFPrintService.prototype = {
  ...
  useRenderedPage: function useRenderedPage(printItem) {
    ...
    if ('toBlob' in scratchCanvas && !this.disableCreateObjectURL) {
     ...
     img.src = scratchCanvas.toDataURL(undefined, this.isMobile ? 0.7 : undefined); // Reduce quality for mobile
